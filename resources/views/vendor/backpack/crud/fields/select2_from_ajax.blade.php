                ...
                processResults: function (data, params) {
                    ...
                    var result = {
                        results: $.map(data.data, function (item) {
                            ...
                            textField2 = textField.split('.');
                            if(textField2.length>1){
                                let camelToSnakeCase = camelToUnderscore(textField2[0]);
                                return {
                                    text: item[camelToSnakeCase][textField2[1]]['{{app()->getLocale()}}'],
                                    id: item[$connectedEntityKeyName]
                                }
                            }
                            return {
                                text: item[textField]['{{app()->getLocale()}}'],
                                id: item[$connectedEntityKeyName]
                            }
                        }),
                        pagination: {
            ...
            $selectedOptions != [])
        {
            ...
            select2AjaxFetchSelectedEntry(element).then(function(result) {
                result.forEach(function(item) {
                    ...
                    $(element).append('<option value="'+$itemValue+'">'+$itemText['{{app()->getLocale()}}']+'</option>');
                });
